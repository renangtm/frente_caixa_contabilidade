...
public class ExpedienteCaixaService implements Service<ExpedienteCaixa> {
	...
	public static void main(String[] args) {
		...
		List<Movimento> movimentos = serv.getMovimentosSemReducao(exp);
	...
	public List<Movimento> getMovimentosSemReducao(ExpedienteCaixa exp) {

		return (List<Movimento>) (List<?>) et.createQuery(
				"SELECT m FROM Movimento m WHERE m.expediente=:expediente AND m.id NOT IN (SELECT ms.id FROM Reducao r JOIN FETCH r.movimentos ms)")
				.setParameter("expediente", exp).getResultList();
	...
	public ExpedienteCaixa merge(ExpedienteCaixa e) {
		...
		e.setAjustes(e.getAjustes().stream().map(a -> {
			...
			ee.setSaldo_final_atual(ee.getSaldo_final_atual()
					+ (a.getValor() * (a.getTipo() == TipoAjuste.FISICAMENTE_AMAIOR ? 1 : -1)));

			return a;

		}).collect(Collectors.toList()));

		e.setReducoes(e.getReducoes().stream().map(r -> {
		...
		e.getMovimentos().forEach(m -> {
			...
			if (m.getFormaPagamento().equals(FormaPagamentoNota.DINHEIRO)) {
				...
				ee.setSaldo_final_atual(
						ee.getSaldo_final_atual() + (m.getValor() * (m.getOperacao().isCredito() ? 1 : -1)));
		...
		e.getReducoes().forEach(r -> r.setExpediente(ne));
