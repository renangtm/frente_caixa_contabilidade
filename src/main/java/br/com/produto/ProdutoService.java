...
public class ProdutoService implements Service<Produto> {
	...
	public List<ProdutoRelatorio> getProdutoRelatorio(String filtro, Calendar inicio, Calendar fim) {
		...
		Map<Integer, List<ProdutoNota>> entradas = new HashMap<Integer, List<ProdutoNota>>();

		List<ProdutoNota> pn = et
				.createQuery(
						"SELECT pn FROM ProdutoNota pn WHERE pn.nota.operacao=:operacao AND pn.nota.empresa=:empresa ORDER BY pn.nota.data_emissao DESC")
				.setParameter("operacao", SaidaEntrada.ENTRADA).setParameter("empresa", this.empresa).getResultList();

		for (ProdutoNota p : pn) {

			List<ProdutoNota> l = entradas.get(p.getProduto().getId());

			if (l == null) {

				l = new ArrayList<ProdutoNota>();
				entradas.put(p.getProduto().getId(), l);
		...
		for (Object[] linha : (List<Object[]>) (List<?>) q.getResultList()) {
			...
			List<ProdutoNota> ents = entradas.get(produto.getId());

			if (ents != null) {

				double qq = pr.getQuantidade();

				for (int i = 0; i < ents.size() && qq > 0; i++) {

					ProdutoNota e = ents.get(i);

					qq -= e.getQuantidade();

					double qr = e.getQuantidade();
					...
					pr.setIcms(pr.getIcms() + ((qr / e.getQuantidade()) * e.getImposto().getIcms().getValorIcms()));

				}

			} else {

				pr.setIcms(pr.getValor() * pr.getQuantidade() * 0.028);
			...
			try {
				...
				if (mp.getData().getTime() > fim.getTimeInMillis()) {
					...
					pr.setQuantidade(pr.getQuantidade()
							+ ((mp.getOperacao().equals(SaidaEntrada.SAIDA) ? 1 : -1) * mp.getQuantidade()));
	...
	public Produto getProduto(String codigo) {
		...
		@SuppressWarnings("unchecked")
		List<Produto> produtos = (List<Produto>) (List<?>) et
				.createQuery("SELECT p FROM Produto p WHERE p.codigo_barra = :codigo").setParameter("codigo", codigo)
				.getResultList();
	...
	public int getCount(String filtro) {
		...
		Query qr = et
				.createQuery("SELECT COUNT(*) FROM Produto p WHERE p.empresa.id = :empresa AND (p.nome LIKE :nome) "
						+ (this.isComEstoque() ? " AND p.estoque.disponivel>0" : ""));
	...
	public List<Produto> getElementos(int x1, int x2, String filtro, String ordem) {
		...
		Query qr = et.createQuery("SELECT p FROM Produto p WHERE p.empresa.id = :empresa AND (p.nome LIKE :nome)"
				+ (this.isComEstoque() ? " AND p.estoque.disponivel>0" : "")
