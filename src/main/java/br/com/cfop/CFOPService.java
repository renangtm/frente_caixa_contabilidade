...
public class CFOPService implements Service<CFOP> {
	...
	public int getCount(String filtro) {
		...
		Query q = this.et
				.createQuery("SELECT COUNT(*) FROM CFOP n WHERE n.numero LIKE :numero OR n.descricao LIKE :desc");
	...
	public List<CFOP> getElementos(int x1, int x2, String filtro, String ordem) {
		...
		Query q = this.et.createQuery("SELECT n FROM CFOP n WHERE n.numero LIKE :numero OR n.descricao LIKE :desc"
				+ (!ordem.equals("") ? " ORDER BY " + ordem : ""));
		q.setParameter("numero", "%" + filtro + "%");
		q.setParameter("desc", "%" + filtro + "%");
		q.setFirstResult(x1);
	...
	public CFOP getPeloNumero(String str) {

		Query q = this.et.createQuery("SELECT n FROM CFOP n WHERE n.numero = :numero");
		q.setParameter("numero", str);
