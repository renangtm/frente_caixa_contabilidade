...
public class NCMService implements Service<NCM> {
	...
	public int getCount(String filtro) {
		...
		Query q = this.et
				.createQuery("SELECT COUNT(*) FROM NCM n WHERE n.numero LIKE :numero OR n.descricao LIKE :desc");
	...
	public List<NCM> getElementos(int x1, int x2, String filtro, String ordem) {
		...
		Query q = this.et.createQuery("SELECT n FROM NCM n WHERE n.numero LIKE :numero OR n.descricao LIKE :desc"
	...
	public NCM getPeloNumero(String str) {

		Query q = this.et.createQuery("SELECT n FROM NCM n WHERE n.numero = :numero");
