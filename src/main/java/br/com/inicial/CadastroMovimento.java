...
public class CadastroMovimento extends Modulo {
	...
	public void init(Usuario usu) {
		...
		this.comboBox.addActionListener(a -> {
			...
			this.gr = new GerenciadorLista<Movimento>(Movimento.class, this.tblMovimentos, serv, null, new Movimento(),
					RepresentadorMovimento.class);
			...
			ListModelGenerica<Fechamento> lf = new ListModelGenerica<Fechamento>(fs.getPeloBanco(this.banco),
					Fechamento.class, RepresentadorFechamento.class);
		...
		List<Historico> historicos = (List<Historico>) (List<?>) et.createQuery("SELECT h FROM Historico h")
				.getResultList();

		this.cboHistorico
				.setModel(new DefaultComboBoxModel<Historico>(historicos.toArray(new Historico[historicos.size()])));

		List<Operacao> operacoes = (List<Operacao>) (List<?>) et.createQuery("SELECT o FROM Operacao o")
				.getResultList();
		...
		this.btNovoMovimento.addActionListener(x -> {
			...
			try {
				...
				m.setExpediente(et.merge(ConfiguracaoExpediente.getExpedienteCaixa(this.operador)));
		...
		this.btnConfirmar.addActionListener(a -> {
			...
			if (!vc(this.txtValorMovimento) || !vc(this.txtDataMovimento)) {
				return;
			}
			...
			ms.mergeMovimento(this.movimento, true, (p, o, m) -> {
				...
				if (p >= 0) {
					...
					if (p == 100) {
						...
						ListModelGenerica<Fechamento> lf = new ListModelGenerica<Fechamento>(
								fs.getPeloBanco(this.banco), Fechamento.class, RepresentadorFechamento.class);
						this.tblFechamento.setModel(lf);
		...
		this.btnConfirmarBanco.addActionListener(a -> {
			...
			if (!vc(this.txtAgencia) || !vc(this.txtConta) || !vc(this.txtSaldo)) {
				return;
			}
		...
		this.btnExcluirMovimento.addActionListener(x -> {
			...
			if (this.tblMovimentos.getSelectedRow() >= 0) {
				...
				Movimento m = esp.find(Movimento.class,
						this.gr.getModel().getListaBase().get(this.tblMovimentos.getSelectedRow()).getId());
				...
				ms.deleteMovimento(m, true, (p, o, mv) -> {
		...
		apagar = new GerenciadorLista<Conta>(Conta.class, this.tblContasPagar, csp, null, new Conta(),
				RepresentadorConta.class);
		...
		areceber = new GerenciadorLista<Conta>(Conta.class, this.tblContasReceber, csr, null, new Conta(),
				RepresentadorConta.class);
	...
	public CadastroMovimento() {
		...
		panel_4 = new JPanel();
		panel_4.setBorder(
				new TitledBorder(null, "Contas a Receber", TitledBorder.LEADING, TitledBorder.TOP, null, null));
